...
export default async function ReportesPage({
  ...
  const [enrollments, deptEnrollments, attempts, certificates] = await Promise.all([
    ...
    prisma.enrollment.findMany({
      ...
      select: { status: true, progressPercentage: true, user: { select: { id: true, department: true } } },
    }),
    prisma.quizAttempt.findMany({
      where: { finishedAt: { not: null }, ...(quizCurso ? { quiz: { courseId: quizCurso } } : {}) },
      orderBy: { finishedAt: "desc" },
      include: { user: true, quiz: { select: { title: true, passingScore: true, course: { select: { title: true } } } } },
      take: PREVIEW_LIMIT,
    }),
  ...
  type DeptStats = { students: Set<string>; total: number; completed: number; active: number; failed: number; progressSum: number };
  ...
  const deptRows = [...byDept.entries()].sort((a, b) => a[0].localeCompare(b[0]));
            ...
            <TableBody>
              ...
              {deptRows.map(([dept, stats]) => (
                <TableRow key={dept}>
                  ...
                  <TableCell className="text-center text-muted-foreground">{stats.students.size}</TableCell>
